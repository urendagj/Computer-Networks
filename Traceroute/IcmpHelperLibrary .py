...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            #Compare the sequence number between the server and the client
            if self.getPacketSequenceNumber() == icmpReplyPacket.getIcmpSequenceNumber():
                icmpReplyPacket.setIsValidSequencenum(True)
            else:
                print("Invalid Sequence Number", self.getPacketSequenceNumber(), icmpReplyPacket.getIcmpSequenceNumber())
                #Compare the PacketIdentifier between server and client
            if self.getPacketIdentifier() == icmpReplyPacket.getIcmpIdentifier():
                icmpReplyPacket.setIsValidIdentifier(True)
            else:
                print("Invalid Packet Identifier", self.getPacketIdentifier(), icmpReplyPacket.getIcmpIdentifier())
                #Compare Rawdata between server and client
            if self.getDataRaw() == icmpReplyPacket.getIcmpData():
                icmpReplyPacket.setIsValidRawData(True)
                 #Set response to true if all comparisons evaluate to true
                icmpReplyPacket.setIsValidResponse(True)
                pass
            else:
                print("Invalid Raw Data", self.getDataRaw(), icmpReplyPacket.getIcmpData())
            ...
            try:
                ...
                if timeLeft <= 0:
                    ...
                    if icmpType == 11:                          # Time Exceeded
                        print("  Hop=%d    RTT=%.0f ms    Type=%d    Code=%d    %s" %
                                (
                                    self.getTtl(),
                        ...
                        #Print out the correct ICMP errors for the icmpCode
                        if icmpCode == 0:
                        ...
                        #Print our the corresponding erros for the icmpCode
                        if icmpCode == 0:
                            ...
                            print("Communication with Destination Netowork is Administratively Prohibited")
                        elif icmpCode == 10:
    ...
    class IcmpPacket_EchoReply:
        ...
        #                                                                                                              #
        # ############################################################################################################ #
        def printResultToConsole(self, ttl, timeReceived, addr, datapacket):
            bytes = struct.calcsize("d")
            timeSent = struct.unpack("d", self.__recvPacket[28:28 + bytes])[0]
            #Set our PacketDrops flag to false by default
            IsPacketDrops = False
            ...
            #Append the Code and Type values into the arrays for later use
            rttarr.append((timeReceived - timeSent) * 1000)
            Codearr.append(self.getIcmpCode())
            Typearr.append(self.getIcmpType())
            
            #If our sequence number validity check is false, then we display the difference and set our PacketDrops flag to True
            if self.isValidSequencenum() == False:
                print("Received Sequence Number: ", self.getIcmpSequenceNumber())
                print("Expected Sequence Number: ", datapacket.getPacketSequenceNumber())
                IsPacketDrops = True

            #If the valid identifier is not true, then we display the difference and set our PacketDrops flag to True
            if self.isValidIdentifier() == False:
                print("Received Identifier: ", self.getIcmpIdentifier())
                print("Expected Identifier: ", datapacket.getPacketIdentifier())
                IsPacketDrops = True

            #If our Rawdara is not valid, then we display the difference and set our PacketDrops flag to True
            if self.isValidRawData() == False:
                print("Received Raw Data: ", self.getIcmpData())
                print("Expected Raw Data: ", datapacket.getDataRaw())
                IsPacketDrops = True

            #Incriment for packet drops if our flag is set to True
            if IsPacketDrops == True:
                PacketDrops[0] += 1
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        #Display our statisitcs

        print("High RTT: %.0f ms" % max(rttarr))
        print("Average RTT: %.0f ms" % mean(rttarr))
        print("Min RTT: %.0f ms" % min(rttarr))
        print("Packet Drop Percentage: %.0f " % PacketDrops[0])
        

    def __sendIcmpTraceRoute(self, host):
        print("sendIcmpTraceRoute Started...") if self.__DEBUG_IcmpHelperLibrary else 0
        # Build code for trace route her
        ...
        for i in range(30):
            ...
            #If Type and Code are 0, we know that we have rechead the end
            if len(Codearr) > 0 and len(Typearr) > 0:
                if Codearr[0] == 0 and Typearr[0] == 0:
                    break
...
def main():
    ...
    #icmpHelperPing.traceRoute("www.stella-net.net")
